...
// 
register_nav_menus();
// Xoá tiêu đề trang wp-admin bar
...
function memory_page_navi( $args = array() ){
	...
	$args = wp_parse_args( $args, array(
		...
		'first_text'                   => __( 'Trang chủ', 'TinhDk' ),
		'last_text'                    => __( 'Trang cuối', 'TinhDk' ),
		'next_text'                    => __( '&raquo;', 'TinhDk' ),
		'prev_text'                    => '&laquo;',
		'dotright_text'                => '...',
		'dotleft_text'                 => '...',
		'num_pages'                    => 5,
        ...
        <div class="commentator-comment" id="comment-<?php comment_ID(); ?>">
			<p>
				...
				<?php if ($comment->user_id == '1' or $comment->comment_author_email == get_the_author_meta('user_email',1)) {
						echo '<span class="vip commentator-level">Blogger Nổi bậc</span>';
					}else{
...
Nhận xét của bạn đang được xem xét và sẽ được hiển thị sau!</p><?php endif; ?>
				<?php comment_text(); ?><div class="comment-operation"><?php edit_comment_link( __( 'Chỉnh sửa', 'TinhDk' ), '<span class="edit-link">', '</span>' );?></div>
               	</div>
            </div>
      	</div>
<?php }
...
function feed_read_more($content) {
	return $content . '<p><a rel="bookmark" href="'.get_permalink().'" target="_blank">
Đọc tiếp</a></p>';
}
add_filter ('the_excerpt_rss', 'feed_read_more');
// WordPresspost thumbnail
...
// Tải lên giải pháp xử lý lỗi HTTP
...
// Xóa trình đơn công cụ bên trái
...
// Cửa sổ mới mở ra cho trang web của nhà phê bình
...
// Sửa chỗ trật khớp đầu bảng điều khiển
...
// Thêm trình đơn liên kết
add_filter('pre_option_link_manager_enabled','__return_true');
// Xóa số phiên bản chính
remove_action('wp_head', 'wp_generator'); 
// Thông báo lỗi đăng nhập bảng điều khiển ẩn
